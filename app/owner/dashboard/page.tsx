...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // Fetch KYC data only for main owners (not sub-users)
        if (!userData.is_owner_sub_user) {
          try {
            const response = await apiClient.getMyKYC();
            if (response && response.data && response.data.kyc_status) {
              setKycData(response.data);

              // If KYC is not approved, redirect back to home
              if (response.data.kyc_status !== "approved") {
                router.push("/");
                return;
              }
            } else {
              console.error("Invalid KYC response structure");
              router.push("/");
              return;
            }
          } catch (err) {
  ...
  if (!user || (!user.is_owner_sub_user && !kycData)) {
    return null;
  }
