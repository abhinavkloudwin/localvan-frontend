...
export default function DriversManagement() {
  ...
  const fetchOwners = async () => {
    ...
    try {
      const response = await apiClient.getAllUsers(0, 500);
      ...
      // Filter for primary owners only (not sub-owners)
      const ownerUsers = users.filter((user: any) => {
        let roleStr = "";
        if (typeof user.role === "string") {
          roleStr = user.role;
        } else if (user.role && typeof user.role === "object" && "value" in user.role) {
          roleStr = (user.role as any).value;
        }
        // Only include owners who don't have an owner_id (primary owners)
        return roleStr?.toLowerCase() === "owner" && !user.owner_id;
      });
                  ...
                  {owners.length === 0 && !ownersLoading ? (
                    ...
                    owners.map((owner) => (
                      <option key={owner.id} value={owner.id}>
                        {owner.name} ({owner.email || owner.mobile_number})
                      </option>
                    ))
                  )}
                </select>
