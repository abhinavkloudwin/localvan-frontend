...
import type { User, VehicleType, VehicleCategory } from "@/lib/types";
...
function VehicleTypeManagementContent() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        setUser(userData);
        await Promise.all([fetchVehicleTypes(), fetchStatistics(), fetchCategories()]);
      } catch (error) {
  ...
  const handleCreateVehicleType = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.createVehicleType(formData);
  ...
  const handleEditVehicleType = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.updateVehicleType(selectedVehicleType.id, formData);
